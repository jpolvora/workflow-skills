...

{
  ...
  seedConsumerTree(root, { withLocalSkills: true });
  // no root AGENTS.md
  const result = runNode(['--repo-root', root, '--check', '--json']);
  const data = parseJsonOut(result);
  if (data) {
...

{
  ...
  assert(
    /--section autoload/.test(cfg) && /configure_autoload\.cjs/.test(cfg),
...

{
  ...
  if (data) {
    ...
    assert(
      text.includes('<!-- generated by configure_autoload.cjs -->'),
...

{
  ...
  const check = runNode(['--repo-root', root, '--check', '--json']);
...

{
  ...
  const checkMissing = runNode(['--repo-root', rootMissing, '--check', '--json']);
  ...
  const checkOmitted = runNode(['--repo-root', rootOmitted, '--check', '--json']);
...

{
  ...
  seedConsumerTree(root, { withLocalSkills: true });
  seedConfigExample(root);
  const result = runNode(['--repo-root', root, '--set-autoload', 'false', '--json']);
  const data = parseJsonOut(result);
  if (data) {
    const cfg = JSON.parse(
      fs.readFileSync(path.join(root, '.ws/config.json'), 'utf8'),
...

{
  ...
  runNode(['--repo-root', rootMissing, '--set-autoload', 'true', '--json']);
  const checkMissing = runNode(['--repo-root', rootMissing, '--check', '--json']);
  const dataMissing = parseJsonOut(checkMissing);
  if (dataMissing) {
  ...
  runNode(['--repo-root', rootIncomplete, '--set-autoload', 'true', '--json']);
  fs.writeFileSync(path.join(rootIncomplete, 'AGENTS.md'), '# Custom hub without autoload ref\n', 'utf8');
  const checkIncomplete = runNode(['--repo-root', rootIncomplete, '--check', '--json']);
...

{
  ...
  seedConsumerTree(root, { withLocalSkills: true });
  seedConfigExample(root);
  runNode(['--repo-root', root, '--set-autoload', 'false', '--json']);
  const check = runNode(['--repo-root', root, '--check', '--json']);
