...
try {
  const compileCmd = `node ${scriptPath} --compile`;
  ...
  if (!compiledContent.includes("under `{sharedDir}/memory/`")) {
    throw new Error("Compiled MEMORY.md header missing canonical path token");
  }
  ...
  if (!compiledContent.includes("self_learning.cjs --compile")) {
    throw new Error("Compiled MEMORY.md header must mention self_learning.cjs --compile");
  }

  const matchOutput = execSync(`node ${scriptPath} --match-paths test/test-memory-formatting.js`, {
    cwd: rootDir,
    encoding: "utf-8",
  });
  ...
  if (!templateContent.includes("under `{sharedDir}/memory/`")) {
    throw new Error("MEMORY.md.template missing canonical path token");
  }
  if (!templateContent.includes("self_learning.cjs --compile")) {
    throw new Error("MEMORY.md.template must mention self_learning.cjs --compile");
  }
