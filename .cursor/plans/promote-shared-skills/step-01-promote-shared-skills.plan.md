...
### Package membership (draft — refine in Step 2)

**Workflows (`w`):** `spec-to-pr`, `spec-to-pr-lite`, `00-write-spec` … `11-ship-pr`, `github-provider`, `azure-devops-provider`, `local-spec-provider`, `check-harness`, `check-workflows`, `caveman`, `gabarito`, `karpathy-guidelines`, `spec-format`, `goal-loop`, `self-learning`, `changelog` + shared hub.

**Extra (`e`):** `security-review`, `dotnet-security-performance-review`, `tdd-sdd-ddd-reviewer`, `domain-review`, `multi-domain-review`, `secrets-leak-review`, `mobile-first-design`, `taste-skill`, `write-a-skill`, plus promoted utilities when installed standalone (`caveman`, `gabarito`, `karpathy-guidelines`, `spec-format`, `self-learning`, `changelog`) — **without** orchestrators or pipeline steps.

**Full (`f`):** All installable top-level skills + shared hub.
...
1. **Workflows vs Extra boundary:** Should `check-harness` / `check-workflows` be Workflows-only or also Extra? (Draft: Workflows — required for post-install validation in workflow docs.)
2. **Review skill hard deps:** Should `security-review` declare optional vs required `karpathy-guidelines` install dep, or remain autoload-only? (Affects Extra lean install.)
3. **`install-skills.sh`:** Update in same PR or defer? (Site still links cURL installer — risk of drift.)
4. **Transitive deselect UX:** Confirm spec rule “deps stay selected on parent deselect” is final (recommended; no cascade-off).
5. **Azure DevOps provider in Workflows default:** Currently in full pipeline tests; include in `packages.workflows` by default? (Draft: yes — matches `test-install.js` pipeline list.)

**Plan notes:** Complexity = **complex**. Expect **Step 2 interview** to finalize package membership lists and open questions 1–2, then **Step 3 plan-to-tasks** DAG for parallel link-sweep vs CLI workstreams.
