...
endedAt: "2026-09-05T02:08:17.283Z"
acRefs: []
---
...
| B5 | D1 is prose-only (no new intent id) → contract Required intents and parity hardcoded list untouched. | confirmed (design; interview G1: wording locked) |
...
| S12 orch wiring | **1 gap (D1)** | STEP-DISPATCH Steps 0/5/7/8/9 wired; **Step 4 row lacks class-sweep reminder** (verified: no `sibling`/`class sweep`/`Fix-Entire` match in file) |
...
- [x] E1 edit minimal (Step 4 row clause only); E2 matrix re-confirmed per AC.
- [x] `npm run test` exit 0; integrity generate+verify exit 0; harness 0 critical; workflows sim 0 critical.
- [x] Delivery commit skipped per close gate (not fullMode); refined plan stays in worktree.
- [x] No edits under `{globalSkillsRoot}`; no absolute paths; en-us; no host names.
- [ ] PR created via `ws-ship-pr` (`workflowMode`, `stopBeforeFixPr`); local review pass recorded; PR left unmerged.

## 8. Open Questions

None remaining. Interview Round 1 closed every §8 item from project evidence (see Interview registry). `softSkipEligible` was false; interview still ran; no `needs_user` (zero blocking gaps; worker auto-gate takes Recommended: End refinement and advance).

## Interview registry

| id | class | section | gap | status | resolution | resolutionSource | evidence | dependsOn |
|----|-------|---------|-----|--------|------------|------------------|----------|-----------|
| G1 | non-blocking | §2 / S12 | D1 wording minimality: is the appended Step 4 clause lean and orch-owned (no skill-behavior duplication)? | closed | Locked: append-only clause `skill owns Fix-Entire-Defect-Class (repo-wide sibling sweep + exemptions); orch confirms the reminder ran before verify.` Skill behavior stays in `ws-implement-tasks`; no new intent id. | project | `STEP-DISPATCH.md` Step 4 Action cell (no `sibling`/`Fix-Entire` match = gap real); `ws-implement-tasks/SKILL.md` lines 51, 73; `ws-code-review/SKILL.md` line 77 | |
| G2 | non-blocking | §1 B2 | Branch strategy: from-current tip vs rebase-to-main at ship. | closed | Stay on `feat/hermes-spec-to-pr-enhancements` (from-current); no rebase. Ship diff review scopes to `files_touched` only; PR head `feat/...` base `main`. | project | state `branchStrategy: from-current`; worker constraints forbid touching unrelated dirty files; `git rev-list origin/develop..HEAD = 0` (tip == develop tip) | |
| G3 | non-blocking | §2 E2 | Whether E2 finds a rename regression that widens scope beyond D1. | closed | No widening unless Step 4 file:line reads prove a regression. Pre-checks green: class language present in both skills, `run_sabotage.py` on disk, STEP-DISPATCH gap confirmed as the single delta. Full matrix still runs in Step 4/5. | project | greps above; `run_sabotage.py` exists; contract/parity re-verified in E2 | |
