...
status: completed
refined: 2026-09-19
interview: step-02-us-353.plan-interview.md
resolutions:
  - "G1: AC1 mandate locked in test/test-worker-turn-guard.js (both dispatch files)"
  - "G2: no bare-id example on loop path; AC3 adds preventive state-path example + assertion"
  - "G3: mandate sentence added to STEP-DISPATCH.md verbose block"
  - "G4: shape-based payload bound (no byte limit)"
  - "G5: new assertions stay synchronous"
step: 2
workflowId: us-353-20260919T043606Z
startedAt: "2026-09-19T05:05:00.000Z"
endedAt: "2026-09-19T05:37:44.809Z"
acRefs: []
---
...
- Tests layer:
  ...
  - `test/test-worker-turn-guard.js`: extend the AC1 companion block with the continuation-mandate match on both `PROTOCOLS.md` and `STEP-DISPATCH.md` (interview G1 — this file already locks PROTOCOLS phrasing, so it owns the mandate lock).
