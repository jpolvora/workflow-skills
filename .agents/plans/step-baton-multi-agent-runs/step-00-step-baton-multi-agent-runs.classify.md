...
runInterview: true
classifiedAt: 2026-09-18T23:27:22.712Z
scoreAndRefine: false
---

# Pipeline Classification — Step-Level Baton Handoffs for Multi-CLI Workflow Runs
...
|---|---|---|
| pipeline | `standard` | Exceeded threshold(s): implementation steps (ACs) — default recommendation is `standard`. Complexity class `complex`: uncertain or non-simple scope defaults to standard or higher. |
...
| estimatedElapsedSec | `1185` | Sourced from completed-run telemetry median. |
...
|--------|-------|-----------|--------|
| Implementation steps (ACs) | 17 | 3 | no |
| Estimated files (path refs) | 2 | 6 | yes |
...
Exceeded threshold(s): implementation steps (ACs) — default recommendation is `standard`. Complexity class `complex`: uncertain or non-simple scope defaults to standard or higher.
