...
- AC4: The global directory is resolved consistently across operating systems (prioritizing `GEMINI_CONFIG_DIR`, `WORKFLOW_SKILLS_GLOBAL_DIR`, or defaulting to user profile global agent roots `~/.gemini/config` or `~/.agents`).
...
- Precedence chain: explicit user turn > project-level `.agents/skills/` (and `.agents/skills/ws-shared/`) > global skills (`~/.gemini/config/skills` or `~/.agents/skills`).
