...
- `workflow-state.schema.json`: add optional `prNumber`, `prUrl` string fields
  (check schema file shape first; keep `additionalProperties` policy intact).
...
- `STEP-DISPATCH.md` dispatch-provenance paragraph: `agentType:
  ...
  mismatch. Check `WORKER-TURN-RULES.md` parent-contract line first; one home
  for the note, pointer from the other.
