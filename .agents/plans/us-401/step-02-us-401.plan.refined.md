...
## Interview resolutions (Step 2, autoMode model-inferred)

- G1 assumed-default: rebase own commits onto the new tip; merge-forward where
  the host forbids rebase. Helper never executes either; it validates and prints.
- G2 assumed-default: committed regression test is the AC7 enforcement vehicle.
- G3–G7 closed by construction (single canonical list, verified relative links,
  additive schema fields, shared dual-write helper, recipe-scoped scanner).
- `shared_understanding: confirmed`. Sequential execution (Step 3 decision).
