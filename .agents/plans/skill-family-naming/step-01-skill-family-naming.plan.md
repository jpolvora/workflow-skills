---
superseded: true
supersededBy: step-02-skill-family-naming.plan.refined.md
...
endedAt: "2026-09-02T21:55:00Z"
acRefs: []
---
# Implementation Plan — Regroup packaged skill IDs as ws-{family}-{verb}
...
- Rename folder `.agents/skills/ws-write-spec` to `.agents/skills/ws-spec-write` (AC1, AC12).
  ...
  - Update load banner and internal references to `ws-spec-write`.
- Rename folder `.agents/skills/ws-sync-spec` to `.agents/skills/ws-spec-update` (AC1, AC11, AC12).
  ...
  - Clarify distinction between `ws-spec-update` (feature spec update) and `ws-spec-index` (PRD index promotion).
- Rename folder `.agents/skills/ws-multi-spec` to `.agents/skills/ws-spec-multi` (AC1, AC12).
...
- Rename folder `.agents/skills/ws-local-spec-provider` to `.agents/skills/ws-spec-provider-local` (AC2, AC12).
  ...
  - Update internal scripts and docs.
- Rename folder `.agents/skills/ws-github-provider` to `.agents/skills/ws-spec-provider-github` (AC2, AC12, AC14).
  ...
  - Ensure fetch-to-spec invokes `ws-spec-write` and `ws-spec-provider-local` register (AC14).
- Rename folder `.agents/skills/ws-azure-devops-provider` to `.agents/skills/ws-spec-provider-azure-devops` (AC2, AC12, AC14).
  ...
  - Ensure fetch-to-spec invokes `ws-spec-write` and `ws-spec-provider-local` register (AC14).
- Verify `config.json` retains `providers.active` and `providers.scm` enum values `github`, `azure-devops`, `local` (AC15).
- Expected test: V02:provider-family-moved

### 2.3 T03: Plan Family Moves
- Rename folder `.agents/skills/ws-write-plan` to `.agents/skills/ws-plan-write` (AC3, AC12).
...
- Rename folder `.agents/skills/ws-verify-plan` to `.agents/skills/ws-plan-verify` (AC3, AC12).
...
- Rename folder `.agents/skills/ws-update-plan-implementation` to `.agents/skills/ws-plan-update` (AC3, AC12).
...
- Rename folder `.agents/skills/ws-interview` to `.agents/skills/ws-plan-interview` (AC4, AC12).
...
- In `.agents/skills/ws-spec-to-pr/STEP-DISPATCH.md`:
  - Update step dispatch paths, commands, and subagent prompts to reflect the new IDs (AC6).
