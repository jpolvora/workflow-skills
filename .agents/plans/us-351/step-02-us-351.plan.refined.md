...
## Refinements applied (Step 2 interview, findings #7/#9/#13/#15)

- Integrity regen runs from a clean tree: untracked skill-tree files moved aside first, digest
  change confirmed in the commit (integrity-clean-tree trap).
- `tools.md` default change ships with the `ws-check-harness` executable TOKENS mirror update in
  the same batch, plus effective-resolution wording (token-contract trap).
- Installer log lines and skill prose stay generic: no `us-351` / issue numbers in shipped text
  (portable-prose trap).
- Test discovery rule: run the full suite (no `-k`/`--deselect` narrowing), fix every red
  path-asserting test in-batch; post-verify resweep before commit (stale-test + resweep traps).
