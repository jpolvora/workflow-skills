...
interview: step-02-code-review-round-2-fixes.plan-interview.md
draft: step-01-code-review-round-2-fixes.plan.md
acRefs: []
---
> Step 2 interview refinements (see `step-02-code-review-round-2-fixes.plan-interview.md`, registry
> G1–G10, `blocking_open: 0`): Task 3 pins `shell:false` with intact argv; Task 5 is scoped to the
> per-file scan root (the runtime loader is already global-aware); Task 7/11/13/14/15 start with a
> glob locate-first check; Task 8 adds an explicit schema-diff enumeration check; Task 10 names the
> `npm run tests` chain as the wiring point. All other tasks unchanged from the Step 1 draft.
...
- **Actions**: set `shell:false` and pass `--cwd` as an intact argv element on win32 (with `shell:true` the argv array is joined into a string, which splits spaced paths); propagate child `stderr` into the failure `reason`.
...
- **Actions**: resolve the per-file **scan root** from consumer context/config (`pathTokens.skillsRoot`, local-or-global resolution); report findings for global-only/relocated installs. The runtime loader in `check_unique_runtime.cjs` already probes the global root — do not rework it; fix only the scan root (and the hardcoded `path.join(repoRoot, '.agents', 'skills')` in `check_hub_separation.cjs`). Construct any retired-path audit string dynamically (e.g. `['.ws','runtime'].join('/')`) so `test-shared-hub-paths.js` stays clean.
...
- **Target files**: `fix_pr_azure_context.cjs`; GitHub + ADO `sweep_prior_work.cjs`; tracker→spec scripts (`github-issue-to-spec.cjs`, `ado-workitem-to-spec.cjs`) + `resolve_spec_path.cjs` call sites and their docs/parsers. **Locate-first**: resolve exact paths via glob at implementation start (provider skill trees).
- **Checks**: confirm each file exists before editing; keep the three sub-actions (apiBase threading, PR-row unification, `--specs-dir` forwarding) independently testable.
...
- **Actions**: diff `config.schema.json` keys against editor bindings first and enumerate the full unbound set (known: `issueTrackers.github.owner` vs written `org`, `fable.auditVerdictsBlockShip` boolean-`false`, `rules.karpathyGuidelines` default); bind `issueTrackers.github.owner`; persist `fable.auditVerdictsBlockShip` `false` as boolean; default `rules.karpathyGuidelines` to packaged `ws-senior-developer` alias; bind remaining workflow-used schema keys; add round-trip + schema-parity asserts (string `"false"` rejected, boolean `false` persisted).
...
- **Actions**: parity asserts run on synthesized PR rows with zero network/auth and fail on empty envelopes; bootstrap test uses a temp global dir (hermetic); observer test asserts the exact refusal and removes `%TEMP%/observer` residue; global-config test asserts the refusal phrase directly; wire `test-configure-auto.js` into the `npm run tests` chain (`package.json` `"test": "npm run tests"`).

### Task 11: Benchmark Node-only paths + Python-residue scan scope (AC11)

- **Target files**: `scripts/harness-benchmark/lib/paths.cjs`; `scripts/harness-benchmark/lib/sensor.cjs` (or equivalent sensor); `test/test-unique-runtime.js`. **Locate-first**: confirm benchmark lib layout via glob at implementation start.
...
- **Target files**: `scripts/harness-benchmark/lib/run_sabotage.cjs` (or packaged `run_sabotage.cjs`); `.agents/skills/ws-spec-to-pr/scripts/check_memory_conflict.cjs`; `cleanup_workflow_git.cjs`; `.agents/skills/ws-monitor/scripts/monitor_snapshot.cjs`. **Locate-first**: confirm `run_sabotage.cjs`, `cleanup_workflow_git.cjs`, and `secrets_scanner.cjs` (Task 14) paths via glob at implementation start.
...
- [x] Layer boundaries respected (skills-sot, installer-cli, tests).
- [x] Domain entities and mappings encapsulated (N/A — no domain model change).
- [x] Schema migrations created (N/A — no database).
- [x] Authorization checks applied (N/A — no auth surface; apiBase threading only).
- [x] Stack & security invariants verified (auth, async, validation, cleanup per §6).
- [x] i18n keys declared (N/A — en-us only).
- [x] Test cases cover all ACs (AC1–AC15 per §5).
