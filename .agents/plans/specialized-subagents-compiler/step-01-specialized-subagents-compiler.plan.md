---
superseded: true
supersededBy: step-02-specialized-subagents-compiler.plan.refined.md
...
endedAt: "2026-09-09T12:23:45.375Z"
acRefs: []
---
...
1. **Configuration Layer:**
   - Schema: `defaults.specializedSubagents` object with `enabled` (boolean, default: `false`), `targetHost` (`auto` | `cursor` | `claude` | `generic`), and `agentPrefix` (string, default: `"ws"`).
...
2. **Compiler Engine (`compile_host_subagents.cjs`):**
   - Resolves canonical skills:
     ...
     - Step 5: `.agents/skills/ws-plan-verify/SKILL.md` -> `ws-step-05-plan-verify.md`
...
4. **Hub Layout & Packaging Manifest:**
   - Update `hub-layout.json` to categorize generated host projections.
...
- Implement frontmatter generator with `name: {prefix}-step-{N}-{role}`, scoped `description: "..."` restricting to workflow orchestrator dispatch, and mandatory `disable-model-invocation: true`.
