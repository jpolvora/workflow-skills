---
superseded: true
supersededBy: step-02-ws-wiki-code-verify.plan.refined.md
...
endedAt: "2026-09-12T16:10:43.873Z"
acRefs: []
---
...
1. **Enumerator helper** (covers AC4, AC5; negatives NS2, NS3, NS9, NS11):
   - Action: write `.agents/skills/ws-wiki/scripts/list_wiki_feature_pages.cjs` by mirroring `list_wiki_sweep_specs.cjs` arg parsing + `assertContained`; implement recursive walk, exclusions, POSIX sort, `--json` shape; `--help` text; exit codes (0 ok/help, 1 containment/runtime, 2 unknown-flag/missing-value, before `readdir`).
...
2. **SKILL.md Phase 2 + Phase 3 prose** (covers AC1–AC3, AC6–AC16; negatives NS1, NS4–NS8, NS10):
   - Action: add subcommand lines + two mode sections + helper usage; encode every gate (post-sweep offer, post-audit offer, per-finding truth choice), every STOP (missing index, apply-without-audit, cancel), read-only and batch rules, checkpoint lifecycle, `--resume`/`--force`, `--dry-run` purity, empty-wiki success, no-orch-start. Keep `user-gate`/`askQuestion` portable aliases only.
...
4. **Tests** (covers AC17; negatives NS1–NS9, NS11):
   - Action: extend `test/test-wiki.js` (or add `test/test-wiki-verify.js` + register in `package.json` `tests:harness-efficiency`); fixture tmp wiki trees per case; assert exit codes, JSON shapes, file mtimes, and string presence.
...
1. **Sibling vs extension for tests** — extend `test/test-wiki.js` in place (simpler, matches AC17's first option) or add `test/test-wiki-verify.js`? Recommendation: extend in place unless the file exceeds ~700 lines, then split and register the sibling in `package.json`. Interview may confirm; default is extend-in-place. (Non-blocking.)
2. **`domain` for root-level pages** — pages directly under `{wikiDir}/*.md` (not in a subfolder): `domain` = `""` vs `"general"`? Recommendation: `""` (empty string) with `feature` = basename; index link still resolves. Interview may confirm. (Non-blocking; test pins whichever is chosen.)
3. **No other blockers** — spec DoR records none; stack can be detected from `config.json`; MEMORY consulted (traps folded: integrity-regen-last, no-benchmark, anchored staging, containment patterns, `>/dev/null` only).
