...
REPO_ROOT = Path(__file__).resolve().parents[4]   # .agents/skills/us-delivery-workflow/scripts -> repo root
MEMORY_PATH = REPO_ROOT / "MEMORY.md"

# Módulos genéricos padrão de desenvolvimento de software.
# Podem ser estendidos localmente via .agents/skills-config.json
DEFAULT_KNOWN_MODULES = [
    "Auth", "Tenant", "User", "Config", "Settings", "Audit",
    "Notification", "Email", "Payment", "Billing", "Invoice",
    "Search", "Filter", "Report", "Export", "Import", "Sync",
    "Cache", "Queue", "Job", "Logging", "Common", "Shared",
    "Security", "Encryption", "Upload", "Storage", "Database",
    "Client", "Customer", "Provider", "Product", "Order",
]

DEFAULT_KNOWN_LAYERS = ["Domain", "Application", "Infrastructure", "UI", "Test", "DevOps", "Core", "API"]

_TOKEN_BOUNDARY_BEFORE = r"(?<![a-z0-9])"
_TOKEN_BOUNDARY_AFTER = r"(?![a-z0-9])"


def load_custom_config(repo_root: Path) -> dict:
    """Carrega dados customizados de módulos e layers do repositório local, se houver."""
    config_path = repo_root / ".agents" / "skills-config.json"
    if config_path.exists():
        try:
            return json.loads(config_path.read_text(encoding="utf-8"))
        except Exception:
            pass
    # Tenta obter do config do Azure DevOps como fallback secundário
    azdo_path = repo_root / ".agents" / "azure-devops.config.json"
    if azdo_path.exists():
        try:
            return json.loads(azdo_path.read_text(encoding="utf-8"))
        except Exception:
            pass
    return {}
...
def extract_plan_keywords(path: Path, known_layers: list[str], known_modules: list[str]) -> dict:
    text = _read_utf8(path)
    ...
    # Capturar caminhos de arquivos relativos genéricos
    ...
    for m in re.finditer(
        ...
        r'|Component|Directive|Module|Dto|Mapper|Model|Helper|Util|Context)\b',
        text,
    ):
...
def main():
    ...
    # Carregar configurações dinâmicas de stack do repositório alvo
    config = load_custom_config(REPO_ROOT)
    
    known_modules = list(set(DEFAULT_KNOWN_MODULES + config.get("knownModules", [])))
    known_layers = list(set(DEFAULT_KNOWN_LAYERS + config.get("knownLayers", [])))

    memory = parse_memory(MEMORY_PATH)
    plan = extract_plan_keywords(plan_path, known_layers, known_modules)
