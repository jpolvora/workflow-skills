...
2. `*.state.md` has `status: completed` **and** terminal `shipStatus` (`skipped` \| `merged` \| `stopped`; or legacy: no `shipStatus` with Step 9 in `completedSteps`), **or** `status: cancelled` \| `failed`, **or** the folder name ends with `.archive`, and
