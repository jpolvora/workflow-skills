...
| 2 | **Memory** | Consult via [`tools.md`](../../ws-shared/runtime/tools.md) **`read-memory`** / [`ws-self-learning`](../../ws-self-learning/SKILL.md) Pre-work for the defect class and touched paths — **same evidence class as code**. When `enableSpecMemoIntegration`: vault MCP/CLI `bootstrap`/`search`. When `enableMemoryFiles`: `{memoryDir}/MEMORY.md` + `memory/*` / `--match-paths`. Dual → vault first, then local. Reuse known `INSTEAD DO` solutions. Per-backend absence or flag-off is **consult-skipped, not fatal** — record `memory-files: consult-skipped` and/or `spec-memo: consult-skipped` in `sourcesConsulted` and continue. |
