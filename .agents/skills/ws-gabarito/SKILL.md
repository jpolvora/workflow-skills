...
| 11 | Anti-regression memory | Before plan/code/fix: Grep `{sharedDir}/MEMORY.md` (expand per `tools.md` Path tokens); apply Solutions. After: write new traps via `ws-self-learning` |

## Opt-out

| Phrase | Effect |
|--------|--------|
| `stop ws-gabarito` / `sem ws-gabarito` | Disable ws-gabarito for this session (hub § Opt-out) |
