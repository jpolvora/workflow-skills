...
3. **Validation & Observation Notes & Negative Scenarios:**
   ...
   - List negative failure scenarios under `### Negative & Failing Test Scenarios`. When a stack rule pack applies (from `{skillsRoot}/ws-shared/runtime/stacks/`), inject at least one negative scenario validating stack safety and invariant compliance (e.g. unauthenticated access rejection, sync-over-async deadlock prevention, or permission gate enforcement).
