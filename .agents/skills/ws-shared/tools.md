...
| `read-memory` | Load learned knowledge **before** plan/code/fix | When `specMemo.enabled` is explicit `true`, prefer `ws-spec-memo` bootstrap / `{specMemo.cli} search`; else `Grep` / `Read` `{sharedDir}/MEMORY.md`. Mandatory for mutating work — see [`ws-self-learning`](../ws-self-learning/SKILL.md) § Pre-work consult |
