...
- **DO NOT**: Keep Extra/optional Post skills in check_workflows `aux_skills`, leave eval-count assertions hardcoded after deleting skill evals, or skip `node bin/build-site.js` after catalog membership changes.
- **INSTEAD DO**: Drop demoted ids from orch dispatch-closure lists; recount eval files (`bin/validate-evals.cjs` stdout); rebuild the site catalog in the same change set as skill-graph edits.
