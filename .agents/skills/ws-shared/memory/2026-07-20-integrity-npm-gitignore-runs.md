...
- **Severity**: `High`
- **Trap Avoided**: (1) `HUB_WHITELIST` included `shared/.gitignore`, and the generator hashed it from the working tree, but `npm pack` always omits files named `.gitignore` — packed installs then failed source integrity (`hub/.gitignore missing`). (2) Local `*/runs/**` artifacts (e.g. `ws-fix-pr/runs/`) were hashed from disk but not present in the tarball → same pre-copy fail.
