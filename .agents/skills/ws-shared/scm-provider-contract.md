...
| `resolve-thread` | thread id (+ comment; optional `--model`; PR id when the host requires it) | Resolved; resolution comment describes the correction (not hash-only) and includes `---\nLLM model: {id}` when `--model` is set | Skip remote mutation when the caller is `dry-run`. Reject comments that are only a commit hash and/or model footer. |
