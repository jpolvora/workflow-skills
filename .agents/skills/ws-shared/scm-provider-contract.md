...
| `resolve-thread` | thread id (+ comment; optional `--model`; PR id when the host requires it) | Resolved; resolution comment includes `---\nLLM model: {id}` when `--model` is set | Skip remote mutation when the caller is `dry-run`. |
