...
4c. **Resume pre-check (unique commits vs base — mandatory before re-implement):** After any branch checkout and before dispatching re-implementation work on a resumed workflow, mechanically verify the feature tip has **non-zero unique commits** versus the recorded base. Run `git rev-list --count origin/{state.baseBranch}..HEAD`. If the count is **NOT > 0** (i.e. the count is `0` — the feature tip has no commits that are not already on `origin/{state.baseBranch}`), the workflow was likely already merged into the base; do **not** re-implement or create/update a PR. Instead **mark the leftover workflow `completed`** in state (already merged), restore HEAD to `{state.baseBranch}` (`git checkout {state.baseBranch}` only — never `reset`, never `-D`), log `resume-gate | stale | 0-unique-commits | {branch} vs {baseBranch} | ISO` in `## Gate history`, and stop so the user starts new work from the base. If the count is `>= 1`, proceed with the normal resume re-implementation. When `origin/{state.baseBranch}` is unavailable (not fetched / git-less dry-run), log `resume-gate | skip-check | {branch} | ISO` and continue (dry-run/git-less never blocks resume).
