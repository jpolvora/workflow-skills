...
Orchestrators resolve the binding at bootstrap (before the first `user-gate` or `dispatch-agent`) in this order: config override `defaults.hostAdapter.mode` when non-`auto` → disk-cache hit in `{sharedDir}/host-capabilities.json` for the current `hostId::orchestratorModel` key → one active probe via `node {skillsRoot}/ws-shared/runtime/scripts/probe_host_capabilities.cjs --key {hostId::orchestratorModel}` (pre-mapped host shapes in `../host-tool-map.json`, unknown hosts degrade to the minimal safe set, never a startup failure). Cache misses upsert only the current key (preserving others); missing/unreadable cache behaves as a miss. Log `host-capability-bind | {json} | {hit|probe} | ISO` to step telemetry JSONL during Step 0. The cached entry is reused across every step — no per-step re-probing; re-probe only on explicit `--refresh`, host/toolset change, explicit rebind, or a different session key. Capability-token vocabulary and the full invalidation rule: [`host-capability-tokens.md`](host-capability-tokens.md). Alias table and full protocol: [`tools.md`](tools.md) § Host-tool binding & dispatch tiers.
