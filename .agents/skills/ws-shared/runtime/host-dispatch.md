...
- **Specialized Named Subagent Dispatch (when enabled):**
  ...
  - **Resolution:** Resolve the host projection directory from the configured `directory` (`projectLevel` dynamically resolved relative to `{repoRoot}`, `userLevel` resolved to the user agent directory, etc.) and configured `targetHost`, then target the step-specific named subagent matching `{prefix}-step-{step}-{role}` (for example, `ws-step-04-implement-tasks`). Never resolve a project projection from the global skill installation.
