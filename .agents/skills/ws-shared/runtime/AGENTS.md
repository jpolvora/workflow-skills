...
| File | Purpose |
|------|---------|
| [`config.json.example`](../templates/config.json.example) | Project config template — seeded to `config.json` on fresh install; fill via [`ws-configure-project`](../../ws-configure-project/SKILL.md); track only when non-secret |
