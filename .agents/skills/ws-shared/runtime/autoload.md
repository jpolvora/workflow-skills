...
| Draft a new local spec or reformulate tracker issue | [`ws-spec-write`](../../ws-spec-write/SKILL.md) | Does not create `{plansDir}` / `step-00`; does not run orch. Standalone: auto-track via `ws-spec-index` `track` (no gate) |
