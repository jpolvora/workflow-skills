...
| `create-pr` | head, base, title/body | PR URL + id | Reuse an existing open PR for the same head→base when present. When a tracker id is present, the caller keeps a `Closes #{id}` line in the PR body (GitHub auto-closes the issue on merge); skip for `id: null` / `source: local`. |
