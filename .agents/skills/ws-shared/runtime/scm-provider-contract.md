...
| `create-pr` | head, base, title/body | PR URL + id | Reuse an existing open PR for the same head→base when present. When a tracker id is present, the caller keeps a `Closes #{id}` line in the PR body (GitHub auto-closes the issue on merge **only when the PR base is the repository default branch**); skip for `id: null` / `source: local`. After merge on any base, callers dispatch `close-issue` for an explicit tracker transition. |
...
| `comment-issue` | tracker id, body (PR URL + summary) | Comment posted (alias `close-loop` in tools.md only) | **Comment-only** — does **not** change tracker state. Skip when `id` is null / `source: local` (exit 0 `skipped`). `dry-run` prints body, no POST. WIT Comments `api-version=7.1-preview.4` on ADO (not PR threads). |
| `close-issue` | tracker id | Closed / skipped / dry-run | Explicit tracker state transition after merge: GitHub `gh issue close`; ADO WIT `System.State` PATCH to `Closed` (fallback `Done` when the process rejects `Closed`). Skip when `id` is null / `source: local` (exit 0 `skipped`). `dry-run` prints planned close, no mutation. `validate-auth` before mutate; no silent provider fallback. |
| `create-issue` | title, body (+ optional labels / work-item type) | New issue/work-item URL + id | Open a new tracker issue/work item carrying an actionable defect report (failure class, the contract or instruction to change, evidence, reproduction shape). Body must be anonymized — no consumer repository names, local paths, hostnames, secrets, tracker ids, transcripts, or customer data. `dry-run` prints the payload, no mutation. `validate-auth` before mutate; no silent provider fallback. Used by the `ws-monitor` stall/defect observer when `--open-issue` is set. |
