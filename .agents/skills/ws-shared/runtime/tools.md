...
7. `{skillsRoot}` is **fixed install layout**, not a relocatable consumer knob (unlike `plans.dir` / `plans.specsDir` / `reviews.dir` / `rules.memoryDir`). `{sharedDir}` resolves to the project hub root (default `.ws`); an explicit `pathTokens.sharedDir` value wins when configured, otherwise the default applies — there is no fallback read of a previous hub location. Hub-hosted content (installer artifacts, autoload, generated consumer skills such as `ws-project-patterns`) resolves under the configured hub via the single resolver `{skillsRoot}/ws-shared/runtime/scripts/resolve_hub_root.cjs`. The bootstrap `config.json` stays fixed at `.ws/config.json` (hub discovery point, never relocates). Hub values must be repo-relative and contained: traversal (`../`), absolute paths, and symlinked escapes are refused fail-closed.
...
10. **Skill-script path expand (hybrid):** for managed script recipes, resolve `{skillsRoot}/ws-<id>/scripts/...` when that path exists under the consumer project; otherwise `{globalSkillsRoot}/ws-<id>/scripts/...` (same local-first rule as `configure_autoload.cjs` `emit_skill_path`). The bootstrap consumer config (`config.json`) always comes from `$PWD/.ws/config.json` (fixed discovery point); `STACK.md` and other hub content come from `$PWD/{sharedDir}` — never from `../ws-shared/` relative links inside a globally installed `SKILL.md` (those point at the global hub on disk). MEMORY/changelog come from the effective `{memoryDir}` / `rules.changelogFile` (defaults: repo root; legacy `{sharedDir}` fallback).
11. **Managed hub content (runtime/templates):** resolve docs, schemas, stacks, and scripts under `{skillsRoot}/ws-shared/runtime/` (project skills install) or `{globalSkillsRoot}/ws-shared/runtime/` (global/hybrid), and seed templates under `{skillsRoot}/ws-shared/templates/` or `{globalSkillsRoot}/ws-shared/templates/` — local-first. The consumer hub (`{sharedDir}`, default `.ws/`) holds consumer config and the generated entrypoint only: **never** resolve `{sharedDir}/runtime/` or `{sharedDir}/templates/`.
...
| `read-config` | Load project config | `Read` `.ws/config.json` (bootstrap, fixed) |
...
| Tool | Action | Native |
|------|--------|--------|
| `commit-code` | Commit workflow product files from `files_touched` | Path-scoped `Shell`: `HEAD` must equal `state.branch`; `git add -- <paths>` and `git add -u -- <deleted-paths>` from workflow `files_touched` (drop `{plansDir}/**`, secrets, gitignored, `preExistingDirty`). Never `git add -A`, `git add .`, bare `git add -u`, bare `git add -u --`, or directory-wide `src/` `web/` `tests/`. When a path is also in `preExistingDirty`, inspect `git diff HEAD -- <path>`, `git diff --cached -- <path>`, and `git status --porcelain -- <path>` to separate staged WIP from unstaged fix hunks, then stage only fix hunks via a non-interactive scoped patch / `git apply --cached` (never interactive `git add -p` in automation). If the index already holds unrelated hunks on that path, unstage them with `git restore --staged -- <path>` while keeping the WIP in the worktree before hunk-staging; if anchor hunks cannot be separated without staging unrelated WIP, do not resolve that 6–10 thread as fixed — leave it open or escalate with `path + reason`. Empty `git diff --cached` → skip, and resolve comment-only only for 0–5 threads. Then `git commit -m "..."`. Ownership contract: [`git-ownership.md`](git-ownership.md) |
