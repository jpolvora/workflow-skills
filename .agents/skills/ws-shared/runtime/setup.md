...
**Spec-before-plan rule (all entries):** every entry path first produces the **spec of record** `{specsDir}/{slug}.spec.md` (via `ws-spec-write` or local file normalization), then the **workflow copy** `{us-dir}/step-00-{slug}.spec.md` (via `ws-spec-provider-local` register). For remote tracker inputs, `ws-spec-write` reformulates the raw issue into an agentic-ready spec with deterministic ACs while retaining original human context. Downstream workflow skills always read `step-00-{slug}.spec.md`.
...
1. **Config check**: Check if `{sharedDir}/config.json` exists.
   - If missing: run [`ws-configure-project`](../../ws-configure-project/SKILL.md) (**Recommended**) — it seeds `{sharedDir}/config.json` from the resolved template source (`{skillsRoot}/ws-shared/templates/` when the project-local skills tree exists; else `{globalSkillsRoot}/ws-shared/templates/` in global/hybrid mode). Do **not** `cp` manually unless you have verified the chosen template directory exists on disk.
...
4. Resume: load `{workflow-id}.state.json` (machine SoT with embedded handoffs), `ac-ledger.json`, and `git log -5 --oneline` on the working branch. Retain `state.modelsPreset` if present so subsequent steps continue using the overridden preset without requiring the parameter to be re-passed. Do not reload every `step-0*.md` up front. **Turn-boundary pause:** when `state.turnPause` is present, resume at `state.turnPause.nextAction` (read `state.stepCheckpoints[state.turnPause.step]` for the remaining units) instead of treating the unchanged `revision` as a stall — the turn ended mid-step; the step's terminating `finish` clears both markers, so a later resume continues the named unit. Then `status: active`, skip bootstrap (including **5b Feature branch gate** — do not re-run), jump to `currentStep` gate.
