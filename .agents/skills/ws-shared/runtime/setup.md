...
1. **Config check**: Check if `{sharedDir}/config.json` exists.
   - If missing: run [`ws-configure-project`](../../ws-configure-project/SKILL.md) (**Recommended**) — it seeds `{sharedDir}/config.json` from the resolved template source (project-local `{sharedDir}/templates/`, global-hybrid `{globalSkillsRoot}/ws-shared/templates/`). Do **not** `cp` from `{sharedDir}/templates/` unless that directory exists on disk.
