...
10. **Patterns & MEMORY Consult:** In Steps 1, 2, and 3: if `defaults.patternsFrontend` is true, read `{sharedDir}/frontend.md` and load `ws-patterns-frontend` before Web/UI edits; if `defaults.patternsBackend` is true, read `{sharedDir}/backend.md` and load `ws-patterns-backend` before backend edits; grep `{sharedDir}/MEMORY.md` for 3–8 plan/spec keywords before coding; record `pattern_consult` and `memory_consult` in step outputs.
