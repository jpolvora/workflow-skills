...
10. **Session leases:** When `defaults.sessionLeases` is not explicit `false` (omitted → on), follow [`setup.md`](../ws-shared/setup.md) §5a (`session_lease.cjs (`acquire` at bootstrap; `heartbeat --lease-id {leaseId}` on each step transition; `release` on terminal status; `git-lock --holder {leaseId}` around destroyable git)` acquire before Step 0/resume; `git-lock` around destroyable git).
