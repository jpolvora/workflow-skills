...
- **Child git cleanup (Phase A):** Successful child workers run mandatory Phase A via their own orch (`ws-spec-to-pr` / lite) when child **shipping is terminal** — shared script `node {skillsRoot}/ws-spec-to-pr/scripts/cleanup_workflow_git.cjs --workflow-id {child-workflow-id}` ([`../ws-spec-to-pr/protocols/artifact-cleanup.md`](../ws-spec-to-pr/protocols/artifact-cleanup.md)). Skipped / failed / aborted children do **not** auto-clean. The batch `runId` is **not** a `uswf/{workflow-id}` cleanup target. Every child follows the ownership-scoped git contract ([`git-ownership.md`](../ws-shared/runtime/git-ownership.md)): stage only its own paths, never stash/revert/clean foreign paths, and advance its `baselineCommit` forward when the base moves.
