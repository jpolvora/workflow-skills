...
| 2 | Prior delivery result for slug | `Glob` `{plansDir}/{slug}/**/step-08-*.result.md` exists and cites merged PR / `merged: true` / `state: MERGED` / `PR #<n>` / `status: completed` | Mark `skipped` + `reason: already-implemented` |
