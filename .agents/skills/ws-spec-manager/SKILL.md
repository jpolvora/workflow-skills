...
**Entry check:** Follow [`config-resolution.md`](../ws-shared/runtime/config-resolution.md) § Entry check. Verify that `.ws/config.json` (bootstrap, fixed) exists in the current repository. If missing or unconfigured, halt and prompt via `user-gate` recommending running `ws-configure-project`.
...
- Read `.ws/config.json` (bootstrap, fixed) via [`config-resolution.md`](../ws-shared/runtime/config-resolution.md).
