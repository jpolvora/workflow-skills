...
Invoke: `/check-harness`, `@check-harness`, or “audit the harness”. Dry-run: `--dry-run` / `dry run`.
...
**Scan:** path token map loaded; Phases 0–5c done; § 3b + retired ids checked when `spec-to-pr` present; Phase 4 hub↔disk diff; Phase 5c context report; zero edits.
