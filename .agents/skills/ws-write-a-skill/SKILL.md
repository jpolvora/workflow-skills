...
2. **Consult memory (`read-memory`)** — Via [`tools.md`](../ws-shared/runtime/tools.md) / [`ws-self-learning`](../ws-self-learning/SKILL.md) Pre-work for skill/script traps (launchers, CRLF, encoding, managed-skill edits) from every enabled backend. Apply Solutions before drafting scripts.
