...
9. **Session leases:** When `defaults.sessionLeases` is not explicit `false` (omitted → on), follow [`setup.md`](../ws-shared/setup.md) §5a: `acquire` before Step 0/resume; persist `{leaseId}` in state; `heartbeat --lease-id {leaseId}` on every post-mutating step transition; `release` on terminal status; `git-lock --holder {leaseId}` around destroyable git (see [`tools.md`](../ws-shared/tools.md)).
