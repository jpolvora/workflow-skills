...
First Step 4 `dispatch-agent` (`ws-implement-tasks`) only after fail-closed `validate_state.cjs --pre-advance 4` exits 0. Guard failure → **HS-5** STOP — no product-file edits, no Step 4 dispatch.
