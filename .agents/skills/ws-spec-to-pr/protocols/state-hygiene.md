...
`--step-output {json_or_file}` passes inline JSON or path to `{plansDir}/{slug}/.runtime/step-{N}-output.json`. When omitted, `update_state.cjs finish` auto-discovers `{plansDir}/{slug}/.runtime/step-{N}-output.json` (or `{us-dir}/step-{N}-output.json`) and falls back to stamped step artifacts if `files_touched` is empty on mutating steps. `--verification-score {score}` is passed when finishing Step 5 (propagates score from `ac_ledger.cjs score`; auto-derived from `{us-dir}/ac-ledger.json` if omitted). `--jsonl-out` is **mandatory** on every call pointing at `{plansDir}/{slug}/telemetry.jsonl` (single append-only stream for all steps). When quality gates are bypassed, run the `bypass` operation with `--gate` and `--reason`.
