...
| 8. [Verify score & SCM providers](#8-verify-score--scm-providers) | Step 5 bar ≥ `defaults.minVerifyScore` (default 9); GitHub/Azure intent parity |
...
*   **Standard (`ws-spec-to-pr`)**: Detailed 10-step lifecycle (0 to 9) containing plan refinement interviews, sequential or parallel DAG execution, read-only verify gates (advance at score ≥ `defaults.minVerifyScore` (default 9)), a required product commit after verify, local code review of `{base}...HEAD`, a second product commit for review fixes, and testing batteries.
...
*   **Role**: Compiles the delivery summary in `step-08-{slug}.result.md` (including step elapsed timing), then presents **the Step 8 user-gate as primary question + overflow** (rule 8; state still records close then ship). Primary: Commit+Create PR (Recommended when `fullMode`) / Commit+Push only / More options…. Overflow: Commit+Skip shipping / Skip commit+Create PR / Separate gates / Pause.
*   **Mechanical flow**: Close-advancing choices run close (G2-delivery per choice, MEMORY, changelog, `status: completed`, `shipStatus: pending`) then ship when applicable. Separate gates / Pause restores separate close-then-ship prompts.
...
Yes. Both implement the required intents in [`scm-provider-contract.md`](../../ws-shared/runtime/scm-provider-contract.md): `validate-auth`, `fetch-to-spec`, `create-pr`, `list-threads`, `sweep-prior-work`, `check-pr-status`, `resolve-thread`, `comment-issue`, `close-issue`, `merge-pr`. Host CLI recipes stay inside each provider `INTENTS.md`. An extra intent on one side without the other (and without an allowlist row) fails `npm run test` (`test/test-provider-parity.js`). [`ws-spec-provider-local`](../../ws-spec-provider-local/SKILL.md) is not an SCM implementer; it delegates PR intents to `providers.scm`.
