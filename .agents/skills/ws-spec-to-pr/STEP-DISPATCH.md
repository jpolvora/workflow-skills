...
| 3 | `dispatch-agent` `ws-plan-to-tasks`; `defaults.enableDag: false` (default) forces `execMode: sequential` (sequential subagent tasks, skip DAG). `defaults.enableDag: true` evaluates `dagThresholds` for parallel DAG tasks. | `step-03-*` when parallel |
