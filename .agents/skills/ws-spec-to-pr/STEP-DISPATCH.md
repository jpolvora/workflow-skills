...
| 8 | **Close implementation then ship — one combined menu, two state phases** ([`gates.md`](../ws-shared/runtime/gates.md)): delivery result → close gate (G2-delivery, MEMORY, changelog, `status: completed`, `shipStatus: pending`) → ship gate → `ws-ship-pr` (`workflowMode: true`, `stopBeforeFixPr: true`, push/PR only). Options: close+PR, close+push, close+skip ship, skip delivery commit+PR, more/pause. **`comment-issue`** on PR create when tracker id present; **`check-pr-status`** for CI triage. | `step-08-{slug}.result.md` |
