...
| 1 | Complexity gate → if simple: stub plan + skip to 4. Else `dispatch-agent` `ws-write-plan`. Run `check_memory_conflict.py` on generated plan. | `step-01-{slug}.plan.md` |
...
| 4 | Pre-implement `check_memory_conflict.py`; `dispatch-agent` `ws-implement-tasks` mode build; verify `pattern_consult`/`memory_consult` proof in step-output; branch-direct default | verification |
