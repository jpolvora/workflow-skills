...
- Missing `config.json` when `{skillsRoot}/ws-shared/templates/config.json.example` exists → **warning** (seed + ws-configure-project).
...
| External companion skills | Hub, task-router, or Always-applied mentions of ids listed in `skill-dependencies.json` → `externalSkills` (e.g. `ws-memo`, `ws-session-tracking` from spec-memo) that have no `SKILL.md` under the skills scan root → **intentional omission** (not `phantom_routes[]` critical). Presence under `{globalSkillsRoot}` is **OK** (hybrid). Do **not** require Layer-table `.agents/skills/<id>/SKILL.md` literals for these ids. Always-applied **must not** list them as mandatory; optional companion section + skip-when-absent is the contract. |
...
| Consumer `config.json` | Missing while `{skillsRoot}/ws-shared/templates/config.json.example` exists → **warning** (seed/copy); placeholders after seed → **suggestion** (`ws-configure-project`), not a broken-link warning |
