...
`{sharedDir}/config.json` → `dagThresholds` (defaults in `{skillsRoot}/ws-shared/templates/config.json.example` when the project-local skills tree exists, else `{globalSkillsRoot}/ws-shared/templates/config.json.example`):
