...
Sequential spec→ship using the **same** pipeline skills as [`spec-to-pr`](../spec-to-pr/SKILL.md). Dual-mode: [`gates.md`](../shared/gates.md) · [`config-resolution.md`](../shared/config-resolution.md) · [`setup.md`](../shared/setup.md). Do **not** use [`STEP-DISPATCH.md`](../spec-to-pr/STEP-DISPATCH.md) for lite step numbers (standard 0–9 only).
...
| State | `python .agents/skills/spec-to-pr-lite/scripts/update_state.py` each step; measured `--elapsed` required; missing telemetry = hygiene fail |
...
For interview, DAG, check-implementation, or Testing: use `/spec-to-pr`.
