...
3. **Score**: link semantic/file/test/alias/sabotage evidence to `{us-dir}/ac-ledger.json`, then run `node {skillsRoot}/ws-spec-to-pr/scripts/ac_ledger.cjs score --ledger <path> --boundary step5`. This derived integer **0-10** is the only score; never author or override a numeric score in a report or state update. Link spec **Negative & Failing Test Scenarios** with `ac_ledger.cjs link --negative NS{n} --test {...}`; uncovered `negativeScenarios` (no observed passing test) are a `knownDefect` and cap the score at 8.
