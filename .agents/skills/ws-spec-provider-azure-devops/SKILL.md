...
```
/ws-spec-provider-azure-devops <intent> [args...]
```
...
- `fetch-to-spec`: spec of record exists at the `resolve_spec_path.cjs` path (enhanced via `ws-spec-write`) **and** `{us-dir}/step-00-{slug}.spec.md` was registered with `source: azure-devops`.
