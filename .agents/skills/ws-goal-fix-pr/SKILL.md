...
| **Runtime storage (AC7–AC8)** | Use the same `$RUNTIME_DIR/revision` and `$RUNTIME_DIR/blocked-reason` contract as [`ws-goal-loop`](../ws-goal-loop/TEMPLATES.md) (prefer `{skillsRoot}/ws-goal-loop/runs/PR-<N>/` or `{skillsRoot}/ws-fix-pr/runs/pr-<N>/`). Fix-round revision increments once per accepted round; blocked reason tracks consecutive identical failure reasons. |
