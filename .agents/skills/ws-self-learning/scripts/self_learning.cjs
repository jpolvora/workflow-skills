...
function compile(context, memoryDir, output) {
  ...
  const entriesRel = toRepoRelative(context.repoRoot, memoryDir, { allowOutside: true });
  const header = [
    '# Memory - Anti-Regression Knowledge',
    '',
    'This file is auto-generated by the `ws-self-learning` skill. DO NOT edit this file directly.',
    `To add new learnings, create a separate markdown file under \`${entriesRel}/\` and run:`,
...
function main() {
  ...
  const effective = resolveEffectiveMemoryPaths({
    repoRoot: context.repoRoot,
    sharedDir: context.sharedDir,
    config: context.config,
  });
  const memoryDir = effective.entriesDir;
  const output = effective.indexFile;
  if (args.compile) {
    if (!routing.enableMemoryFiles) {
      process.stdout.write('Skipped compile (local memory files disabled).\n');
      return;
    }
    return compile(context, memoryDir, output);
  }
