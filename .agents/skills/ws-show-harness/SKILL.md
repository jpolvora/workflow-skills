...
1. **Collect** — From this turn's context only, list what is loaded:
   - Entry: root `AGENTS.md` when authoring upstream; after install load `.ws/AGENTS.md` (optional project root `AGENTS.md` when the host provides one)
   ...
   - Config / external deps only if already known or a one-line path check (`.ws/config.json` (bootstrap, fixed))
