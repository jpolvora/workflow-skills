...
| `ws-show-harness` | Session snapshot of what is loaded | Continuous event log + fix-PR proposal |
...
**End-of-run behavior:** When the audit log contains one or more `error` (or equivalent actionable) findings, the orchestrator (or wrapper skill) MUST present a `user-gate` proposing creation of a PR (or upstream fix PR when running in this package) that repairs the **skill / hub / script content** that caused the runtime defects — not a PR that only papers over consumer project code unless the finding is clearly project-owned. Default gate recommendation: offer to open the fix PR; do not auto-push without confirmation.

**Scope boundary:** Auditing is a **wrapper / observer**, not a replacement for `ws-doctor`, `ws-check-harness`, or `ws-fable-judge`. When `enableAuditing` is false or omitted, orch behavior is unchanged (no audit log obligation, no end-of-run fix-PR gate from this feature).
...
Do not commit audit logs unless the consumer explicitly includes them; default gitignore / Notes guidance may recommend leaving them local.

### Out of scope (v1)

- Replacing static `ws-doctor` or harness Phases 0–5c.
- Auto-merging fix PRs; silent rewrite of managed skills without user-gate.
...
- AC2: When effective `enableAuditing` is `false`, `ws-spec-to-pr` and `ws-spec-to-pr-lite` behavior matches today’s non-audited path (no required audit log; no end-of-run fix-PR proposal from this feature).
...
- AC9: An audit log file is written under `{us-dir}` for the run with structured findings (timestamp, step, skill, category, severity, summary, evidence, recovered flag).
- AC10: At run end, if any actionable `error` findings exist, a `user-gate` proposes creating a PR to fix the runtime skill/hub/script content that caused them; no push/PR without explicit user acceptance.
- AC11: Hub docs (`ws-shared` config comments / resolution notes as needed) and orch skill prose document the flag, the wrapper obligation, and the boundary vs `ws-doctor` / `ws-check-harness` / `ws-fable-judge`.
...
- Related: [`ws-doctor.spec.md`](ws-doctor.spec.md) (static diagnose), `ws-check-harness`, `ws-fable-judge`, `ws-multi-spec`.
...
- Fix PRs from audit findings should target `.agents/skills/ws-*` (or hub templates) in this upstream package when the defect is in managed skill content; consumer trees should recommend upstream PR rather than permanent local edits to managed skills.
