...
- AC9: When a thread fix must edit a path that is already in `preExistingDirty`, the agent inspects `git diff HEAD -- <path>`, `git diff --cached -- <path>`, and `git status --porcelain -- <path>` to distinguish staged WIP from unstaged fix hunks, then stages only the defect-class hunks needed for the thread. If the index already holds unrelated hunks, the agent may unstage them with `git restore --staged -- <path>` while keeping the WIP in the worktree before hunk-staging; if the anchor cannot be separated, the score 6–10 thread stays open or is escalated with `path + reason` rather than resolving without a landed commit.
