...
- **Fixed hub root (enforced, review rounds 1–5):** the hub root is **fixed at `.ws`**. `pathTokens.sharedDir` is *not* a relocation mechanism for hub-hosted content: the installer (`bin/cli.js` `consumerHubDir`), hub layout, migration, and generated link prefixes all assume `.ws`, so honoring a custom root in one script only splits the consumer contract. Earlier review rounds pushed toward honoring it; five consecutive rounds of legitimate findings (writer/reader parity, global context, root template, containment, nested-hub link relativity, installer lifecycle) showed the capability is harness-wide. Decision: keep the hub fixed here, and deliver relocation properly in [`0115-configurable-hub-root.spec.md`](0115-configurable-hub-root.spec.md).
