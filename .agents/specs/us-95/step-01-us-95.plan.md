...
To fix this:
1. Ship `skill-dependencies.json` under `.agents/skills/ws-shared/skill-dependencies.json` as a whitelisted managed hub file during install/update.
2. Update `check_workflows.py` path resolution:
   - Check `.agents/skills/ws-shared/skill-dependencies.json` (consumer install path).
...
4. Update `bin/cli.js` to resolve `skillGraphPath` from `.agents/skills/ws-shared/skill-dependencies.json` or `bin/skill-dependencies.json`.
5. Keep `.agents/skills/ws-shared/skill-dependencies.json` synced with `bin/skill-dependencies.json`.

## 1. Definition of Ready & Scope
- **AC1:** `python .agents/skills/check-workflows/scripts/check_workflows.py` checks `.agents/skills/ws-shared/skill-dependencies.json` first, then falls back to `bin/skill-dependencies.json`.
...
- **AC3:** `HUB_WHITELIST` in `bin/install-rules.js` includes `'skill-dependencies.json'` so `npx workflow-skills install/update` copies `skill-dependencies.json` to `.agents/skills/ws-shared/skill-dependencies.json`.
...
- **`.agents/skills/check-workflows/scripts/check_workflows.py`**:
  - Update `_load_dependencies()` and path constants to resolve `SHARED_DEPS_PATH` (`.agents/skills/ws-shared/skill-dependencies.json`) first, then `BIN_DEPS_PATH` (`bin/skill-dependencies.json`).
...
- **`bin/cli.js`**:
  - Update `skillGraphPath` fallback to check `.agents/skills/ws-shared/skill-dependencies.json` if `bin/skill-dependencies.json` is missing.
- **`.agents/skills/ws-shared/skill-dependencies.json`**:
  - Copy of `bin/skill-dependencies.json` kept in sync.

## 3. Step-by-Step Plan
1. Create `.agents/skills/ws-shared/skill-dependencies.json` as a copy of `bin/skill-dependencies.json`.
2. Update `bin/install-rules.js` to include `'skill-dependencies.json'` in `HUB_WHITELIST`.
3. Update `bin/cli.js` to support resolving `skill-dependencies.json` from `.agents/skills/ws-shared/` or `bin/`.
...
## 5. Test Coverage
- AC1: Test `check_workflows.py` with `.agents/skills/ws-shared/skill-dependencies.json` present.
