...
- **Scenario 6 (Host Ask-mode readonly on Step 5):** Compiling `ws-step-05-plan-verify` with host `readonly: true` opens the verifier in Ask mode, which blocks Shell (tests, `ac_ledger.cjs`) and report writes. The compiler must omit host `readonly` and encode product-tree immutability in the agent body instead.
