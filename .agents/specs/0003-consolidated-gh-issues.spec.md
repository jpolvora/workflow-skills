...
- **Context:** Follow-up to #103. While Phase 4 heading was updated to reference the "resolved hub", residual text in `check-harness/SKILL.md` (lines ~672 and ~720) and `REPORT-FORMAT.md` (lines ~47 and ~57) still hardcodes bare `` `AGENTS.md` ``, causing confusion in consumer mode where the hub is `ws-shared/AGENTS.md`.
- **Target Files:**
...
- **Target Files & Changes:**
  - [.agents/skills/ws-shared/config.json.example](.agents/skills/ws-shared/config.json.example) / `config.json`: Clear `domain.glossaryFile` and `domain.designTokens` to `""`.
  - [.agents/skills/ws-shared/STACK.md.example](.agents/skills/ws-shared/STACK.md.example) / `STACK.md`: Clean up template layer rows to prevent false broken inventory signals.
...
- **Context:** Execution of `check_workflows.py` encounters edge cases:
  ...
  3. Hardcoded references to `.agents/skills` ignore custom `pathTokens` from `ws-shared/config.json`.
- **Target Files:**
...
- **AC2 (#106):** In `.agents/skills/check-harness/SKILL.md`, update L720 DoD checklist text to reference `the resolved hub` (with explicit consumer note for `ws-shared/AGENTS.md`).
...
- **AC4 (#109):** In `.agents/skills/ws-shared/config.json.example` (and seeded `config.json`), set `domain.glossaryFile` and `domain.designTokens` to `""` by default to avoid pointing to non-existent files.
- **AC5 (#109):** In `.agents/skills/ws-shared/STACK.md` (and template), clean up placeholder table rows so check-harness does not treat template markers as broken/incomplete inventory.
...
- **AC9 (#110):** In `check_workflows.py`, load and respect `pathTokens.skillsRoot` and `pathTokens.sharedDir` from `ws-shared/config.json` when present.
