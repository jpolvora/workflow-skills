...
Today, several workflow-agnostic skills live only under `.agents/skills/ws-shared/` (`caveman`, `gabarito`, `karpathy-guidelines`, `spec-format`, `goal-loop`, `self-learning`, `changelog`). They are shipped as nested content inside the `shared/` folder. The interactive installer (`bin/cli.js`) lists top-level directories only, so those nested skills are not first-class installable units. When a consumer installs `spec-to-pr` or `spec-to-pr-lite`, the CLI bulk-copies all of `shared/` (skills plus config/docs) via `ensureSharedInstalled`. Consumers cannot install a single promoted utility (for example `caveman` alone) without taking the whole shared tree.

This feature promotes those seven skill folders out of `shared/` into sibling top-level skill directories under `.agents/skills/` (same level as `spec-to-pr`, `ws-write-spec`, `secrets-leak-review`, etc.). After promotion they appear in the install menu and can be selected individually or via install packages.

**What moves (skill folders):**

| Skill | Current path | Target path |
|-------|--------------|-------------|
| `caveman` | `.agents/skills/ws-shared/caveman/` | `.agents/skills/caveman/` |
| `gabarito` | `.agents/skills/ws-shared/gabarito/` | `.agents/skills/gabarito/` |
| `karpathy-guidelines` | `.agents/skills/ws-shared/karpathy-guidelines/` | `.agents/skills/karpathy-guidelines/` |
| `spec-format` | `.agents/skills/ws-shared/spec-format/` | `.agents/skills/spec-format/` |
| `goal-loop` | `.agents/skills/ws-shared/goal-loop/` | `.agents/skills/goal-loop/` |
| `self-learning` | `.agents/skills/ws-shared/self-learning/` | `.agents/skills/self-learning/` |
| `changelog` | `.agents/skills/ws-shared/changelog/` | `.agents/skills/changelog/` |

**What stays under `.agents/skills/ws-shared/` (not skills; shared config/docs hub):**
...
`shared/` remains the dual-mode config and gate documentation home. It is no longer a container for installable skill packages. Install/update must still ensure `ws-shared/` config/docs are present when any workflow (or workflow package) is installed, without treating `shared/` itself as a skill in the selectable skill list.

**Install packages (installer menu):**

1. **Full package** — select/install every available top-level skill (plus ensure `ws-shared/` config/docs hub).
...
Update root `AGENTS.md`, packaged `.agents/AGENTS.md`, and any routing tables so promoted skills are indexed at their new top-level paths. Remove or rewrite obsolete guidance that forbids top-level `.agents/skills/karpathy-guidelines/` (that note contradicts this promotion). Update relative links inside skills that currently point at `../ws-shared/<skill>/`.

**Preservations (must not regress):**

- Consumer `ws-shared/config.json` remains preserved on update/reinstall.
...
- Dual-mode config resolution continues to use `.agents/skills/ws-shared/config.json` only.
...
- AC1: The seven skills listed above exist as top-level folders under `.agents/skills/<skill>/SKILL.md` and no longer exist as `.agents/skills/ws-shared/<skill>/SKILL.md`.
- AC2: `.agents/skills/ws-shared/` still contains the config/docs hub files (`config.json.example`, `config.schema.json`, `tools.md`, `STACK.md.example`, `setup.md`, `gates.md`, `config-resolution.md`, and related non-skill docs) and does not contain the seven promoted skill folders.
...
- AC5: **Full** selects all installable top-level skills and ensures the `ws-shared/` config/docs hub is installed/updated.
- AC6: **Workflows** selects workflow orchestrators plus their required pipeline/provider/promoted-dependency skills, and ensures the `ws-shared/` config/docs hub is installed/updated. It does not require Extra-only skills that are not workflow dependencies.
...
- AC10: Installing or updating any workflow skill (or the Workflows/Full package) still installs/updates the `ws-shared/` config/docs hub. Consumer `ws-shared/config.json` is never overwritten.
...
- Nested skills under `ws-shared/` are not individually listed.
...
|---------|-------------------------|
| Full | All top-level skills + `ws-shared/` hub |
| Workflows | `spec-to-pr`, `spec-to-pr-lite`, `00`–`11`, providers, `goal-loop`, `spec-format`, `caveman`, `gabarito`, `karpathy-guidelines`, `self-learning`, `changelog`, harness helpers required by workflows, + `ws-shared/` hub |
...
- `spec-to-pr` / `spec-to-pr-lite` → pipeline skills they dispatch + promoted utilities they auto-load + `ws-shared/` hub (hub is not a skill entry).
- `ws-goal-fix-pr` → `goal-loop`, `ws-fix-pr`.
- `ws-write-spec` / providers → `spec-format`.
...
- Relative links of the form `../ws-shared/caveman/SKILL.md` become `../caveman/SKILL.md` (from sibling skills).
- `rules.karpathyGuidelines` default in example config should point at `.agents/skills/karpathy-guidelines/SKILL.md`.
- `self-learning` scripts/docs that hardcode `.agents/skills/ws-shared/self-learning/...` must be updated.
