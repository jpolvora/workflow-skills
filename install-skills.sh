...
echo "  Consumer-owned under .ws/:"
echo "    config.json · STACK.md · installed-skills.json (+ legacy MEMORY.md/memory/, CHANGELOG.md when present)"
echo "    AGENTS.md (hub) · MEMORY/CHANGELOG default to repo root (rules.memoryDir/rules.changelogFile)"
echo "  Managed hub content: {skillsRoot}/ws-shared/runtime/ + templates/ (manifest-driven; never .ws/runtime)"
echo "  Path tokens: config pathTokens + {skillsRoot}/ws-shared/runtime/tools.md ({skillsRoot}/{sharedDir}/{plansDir})"
echo "  (seeded on first install; never overwritten by upstream)"
echo "  Agent contract is .ws/AGENTS.md (no other repo-root files written)"
