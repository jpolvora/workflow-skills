...
| Fail-closed compile: exit 1 and skip rewriting `MEMORY.md` when any entry lacks a dated heading or DO NOT + INSTEAD DO (Node `.cjs` only) | `ws-self-learning` | same |
| Append-only task history | `ws-changelog` | `rules.changelogFile` |
| External vault bridge: configure spec-memo MCP/CLI, import legacy `.agents` tree, dual memory routing (`enableMemoryFiles` / `enableSpecMemoIntegration`), and lifecycle alias translation; day-to-day vault ops stay in `ws-memo` (spec-memo package); plan/implement/fix-pr `read-memory` consults every enabled backend as code-class evidence | `ws-spec-memo` (bridge) + `ws-memo` (runtime) | `~/.spec-memo/projects/<projectId>/` when `enableSpecMemoIntegration: true` |
...
| **Three packages** | `f` Full (all skills), `w` Workflows (47 skills), `e` Extra (`ws-write-a-skill`, `ws-show-harness`, `ws-preview`, `ws-activity-report`, `ws-fable-domain`, `ws-plan-update`, `ws-run-benchmark`, `ws-benchmarks`) |
...
| **Cross-platform** | Node `fs` APIs on Windows, macOS, and Linux; UTF-8 via `LANG`/`LC_ALL` and Node stdio |
| **Script runtime policy** | Node 22 is the unique packaged skill/installer/test runtime. Managed scripts are Node `.cjs` only; no `.py`/`.pyc` (or `__pycache__/` bytecode) is shipped, enforced fail-closed by `ws-check-harness`. Shared `http_retry.cjs` retries 429/5xx |
...
| **Published Wiki** | [jpolvora.github.io/workflow-skills/wiki/](https://jpolvora.github.io/workflow-skills/wiki/) — static HTML generated from the project wiki dir (`.agents/specs/wiki`) on every `node bin/build-site.js` run |
