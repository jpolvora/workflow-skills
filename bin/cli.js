...
function ensureSharedConsumerArtifacts() {
  ...
  if (fs.existsSync(configPath)) {
    ...
    if (fs.existsSync(example)) {
      ...
      console.log(`    Seeded shared/config.json from config.json.example (run ws-configure-project to fill)`);
      ensurePathTokensInConfig(configPath);
    }
  }
...
Interactive package shortcuts:
  ...
  e  Extra package (ws-write-a-skill, ws-show-harness)
...
Notes:
  ...
  - shared/ hub is installed with workflows/full (and when ws-self-learning is installed).
  ...
  - Optional: run the "ws-configure-project" skill to interview/detect and fill shared/config.json placeholders.
...
async function runInstall(skills, opts) {
  ...
  if (installedCount > 0) {
    ...
    console.log('   Optional: run `ws-configure-project` to interview/detect and fill `.agents/skills/shared/config.json`.');
...
function runUpdate(skills, includeNew, forceIntegrity = false) {
  ...
  console.log('   Optional: run `ws-configure-project` if shared/config.json still has placeholders.');
  console.log('   Path tokens: `.agents/skills/shared/tools.md` § Path tokens (`pathTokens` in config.json).');
  process.exit(0);
}
