...
To add new learnings, create a separate markdown file under \`ws-shared/memory/\` and run:
...
function ensureSharedConsumerArtifacts() {
  ...
  if (fs.existsSync(memMd)) {
    ...
    console.log(`    Seeded fresh ws-shared/MEMORY.md (upstream memory not copied)`);
  }
...
Notes:
  ...
  - Consumer-owned under ws-shared/ (never copied from upstream): config.json, MEMORY.md, memory/*, STACK.md, CHANGELOG.md, installed-skills.json, skill-integrity-local.json.
...
async function runInstall(skills, opts) {
  ...
  if (installedCount > 0) {
    ...
    console.log('   Path tokens: `.agents/skills/ws-shared/tools.md` § Path tokens (`pathTokens` in config.json).');
  } else {
