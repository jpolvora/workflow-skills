...
| **Multi-CLI step runs** | Step-level baton runs execute different steps in different CLI processes through the deterministic `step_coordinator.cjs` (no LLM): `defaults.stepRunners` maps steps to runner ids, `defaults.runners` declares command templates, `defaults.stepBaton` tunes polling/retries. Single-writer baton protocol; gates surface at the coordinator. |
