...
* **check-harness awareness:** Changes that add/rename/remove skills or routing tables should note the need to update root `AGENTS.md`, `.agents/skills/ws-shared/AGENTS.md` (+ `.agents/skills/ws-shared/runtime/autoload.md` when the Always-applied set changes), and regenerate `docs/index.html` via `node bin/build-site.js` when applicable.
